...
def unet_permutation_spec() -> PermutationSpec:
    conv = lambda name, p_in, p_out: {f"{name}/kernel": (None, None, p_in, p_out)}
    conv_bias = lambda name, p_in, p_out: {f"{name}/kernel": (None, None, p_in, p_out), f"{name}/bias": (p_out,)}
    norm = lambda name, p: {f"{name}/scale": (p,), f"{name}/bias": (p,)}
    linear = lambda name, p_in, p_out: {f"{name}/kernel": (p_in, p_out), f"{name}/bias": (p_out,)}   # dense, no activation, equal to linear in pytorch
